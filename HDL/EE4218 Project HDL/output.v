...
(
	...
	output 	reg 	inter2_read_en,
	output 	reg 	[inter_depth_bits-1:0] inter2_read_address,
	input	wire 	[width-1:0] inter2_read_data_out,
	...
	// Sums and Counters
	reg [17:0] sum 													= 0;
	reg [$clog2(2**inter_depth_bits):0] 	read_inter2_counter 	= 0;
	...
	begin
		case(State)
			...
			begin
				Done_Output 			<= 0;
				read_inter2_counter 	<= 0;
				read_out_counter 		<= 0;
				write_RES_counter 		<= 0;
				
				inter2_read_address 	<= 0;
				out_read_address 		<= 0;
				RES_write_address 		<= 0;
				sum 					<= 0;
			...
			begin
				inter2_read_en 			<= 1'b1;
				out_read_en 			<= 1'b1;
				inter2_read_address 	<= read_inter2_counter;
			...
			Out_Mult:
			begin
				sum <= sum + (inter2_read_data_out * out_read_data_out);
				...
				begin
					...
					read_inter2_counter <= read_inter2_counter + 1;
					State 				<= Out_Read;
				end
			end
