...
In this project we implemented a simple MLP through software, Hardware Description Language (HDL), and High Level Synthesis (HLS) to work on an FPGA. This MLP takes in a 64x7 input matrix, an 8x2 hidden layer weights matrix, and a 3x1 output layer weights matrix and returns a 64x1 output matrix. In the original template data, the bias component of the weights matrices were the first element. For our implementation, we moved that bias term to the last element. The activation function (in this case, sigmoid) was hardcoded.
...
We used Realterm to send data and receive messages from the board.
